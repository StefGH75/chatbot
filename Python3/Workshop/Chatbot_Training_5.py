...
def preprocess(text):
    ...
    tokens = [word for word in tokens if word not in string.punctuation]  #Satzzeichen entfernen
...
tag_to_response = df.groupby("tag")["responses"].apply(list).to_dict() #Groupierung nach tag, pro Gruppe 
    #interessiere ich mich für die Spalte responses. Diese Werte werden dann in Liste umgewandelt, das wird dann
