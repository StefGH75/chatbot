# Entwickle ein Python-Programm, das ein einfaches Bankkonto-Management-System simuliert. Das Programm soll
# folgende Funktionalitäten umfassen:
# a) Definiere eine Klasse Bankkonto, die Attribute für den Kontoinhaber (inhaber), die Kontonummer (konto_nr),
#  den aktuellen Kontostand (kontostand) und eine Liste der letzten zehn Transaktionen (transaktionen) enthält.
#  Der Kontostand soll mit einem Anfangswert von 0 initialisiert werden, und die Liste der Transaktionen soll 
# leer sein.
...
# c) Implementiere eine Methode abheben, die einen Betrag annimmt und prüft, ob der Betrag vom aktuellen 
# Kontostand abgehoben werden kann. Wenn ja, soll der Betrag abgehoben und die Transaktion gespeichert werden. 
# Wenn nicht, soll eine Ausnahme mit der Nachricht "Nicht genügend Guthaben" geworfen werden.
# d) Füge eine Methode letzte_transaktionen hinzu, die die letzten zehn Transaktionen anzeigt.
# e) Erstelle eine Instanz der Klasse Bankkonto für einen Benutzer und führe verschiedene Einzahlungen und 
# Abhebungen durch. Verwende Ausnahmen, um Fehler wie das Abheben eines Betrags, der den Kontostand übersteigt,
#  zu behandeln.
...
class Bankkonto:
    ...
    def letzte_transaktionen(self):
        ...
        for datum, typ, betrag in self.transaktionen:
            formatiertes_datum = datum.strftime("%d.%m.%Y %H:%M:%S")
            print(f"{formatiertes_datum}: {typ} von {betrag}€")
        #for t in self.transaktionen[-10:]:
         #   print(t)
        print(f"Aktueller Kontostand: {self.kontostand:.2f} €")

konto_1 = Bankkonto("02.02.2023", 1000)
konto_1.einzahlen

def main():
    konto = Bankkonto("Max Mustermann", "DE12345555444")
    while True:
